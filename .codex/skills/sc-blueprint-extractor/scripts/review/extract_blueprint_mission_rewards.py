...
class DataCoreBinaryCompat:
    def __init__(self, raw_bytes: bytes):
        ...
        self.values = {
            ...
            DataTypes.EnumValueName: read_and_seek(dftypes.StringReference * self.header.enum_option_name_count),
        }
...
def resolve_item_labels(item_keys: list[str], english_map: dict[str, str]) -> list[str]:
    return [english_map.get(item_key, item_key) for item_key in item_keys]


def load_datacore_session(*, sc_root: Path, cache_dir: Path) -> DatacoreSession:
    if not sc_root.exists():
        return DatacoreSession(dcb_path=None, status="missing-install", binary=None, note=f"No existe la ruta de Star Citizen: {sc_root}")
    try:
        sc = StarCitizen(sc_root, cache_dir=cache_dir)
        dcb_member = find_datacore_member(sc)
        if dcb_member is None:
            return DatacoreSession(dcb_path=None, status="missing-dcb", binary=None, note="No se encontro Data/Game.dcb ni Data/Game2.dcb en el parche instalado.")
        raw_bytes = sc.p4k.getinfo(dcb_member).open("rb").read()
        compat = DataCoreBinaryCompat(raw_bytes)
        note = f"version={compat.header.version}, trailing_bytes={compat.trailing_bytes}, records={compat.header.record_definition_count}"
        return DatacoreSession(dcb_path=dcb_member, status="ok", binary=bytes(raw_bytes), note=note)
    except Exception as exc:
        return DatacoreSession(dcb_path=None, status="error", binary=None, note=f"{type(exc).__name__}: {exc}")


def probe_datacore_entry(*, session: DatacoreSession, title_key: str, desc_key: str, english_title: str) -> DatacoreProbeResult:
...
def build_markdown_report(
    *,
    ...
    for entry in entries:
        ...
        lines.append(f"- Sonda DataForge: status=`{probe.status}`" + (f", dcb=`{probe.dcb_path}`" if probe.dcb_path else ""))
        lines.append(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Extrae recompensas visibles de blueprints por mision y sondea el DataForge local.")
